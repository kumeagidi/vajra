...
class ParallelConfig:
    ...
    cache_parallel_size: int = field(
        default=1, metadata={"help": "Number of cache parallel groups."}
    )
    max_seq_cache_occupancy: Optional[int] = field(
        default=None,
        metadata={
...
@dataclass
class MnemosyneSchedulerConfig(BaseSchedulerConfig):
    chunk_size: int = field(
        default=512, metadata={"help": "Size of each chunk for Sarathi scheduler."}
    )
    enable_predictive_schedule: bool = field(
        default=False, metadata={"help": "Enable predictive schedule."}
    )
    predictive_schedule_target_batch_execution_latency: Optional[float] = field(
        default=None,
        metadata={"help": "Target batch execution latency for predictive schedule."},
    )
    predictive_schedule_max_chunk_size: Optional[int] = field(
        default=4096, metadata={"help": "Maximum chunk size for predictive schedule."}
    )
    # NOTE(amey): Both of these are hacks to run specific experiments. Do not commit to main.
    skip_execution_till_overlapping: bool = field(
        default=False, metadata={"help": "Skip till overlapping."}
    )
    skip_decode_for_iterations: int = field(
        default=0, metadata={"help": "Number of iterations to skip decoding."}
    )

    def __post_init__(self):
        if not self.enable_predictive_schedule:
            return
        assert self.predictive_schedule_target_batch_execution_latency is not None
        assert self.predictive_schedule_max_chunk_size is not None

    def get_max_num_batched_tokens(self, max_model_len: int):
        # Sarathi never schedules more than chunk_size tokens in one iteration.
        if self.enable_predictive_schedule:
            return self.predictive_schedule_max_chunk_size
        else:
            return self.chunk_size

    @staticmethod
    def get_type():
        return SchedulerType.MNEMOSYNE
...
class MetricsConfig:
    ...
    enabled_append_request_execution_stats: bool = field(
        default=False, metadata={"help": "Enable appending request execution stats."}
    )
...
class WorkerConfig:
    ...
    skip_prefill: bool = field(
        default=False, metadata={"help": "Skip prefilling the model."}
    )
    use_cuda_graph: bool = field(
        default=True, metadata={"help": "Use CUDA graph for the replica."}
    )
    skip_p2p_communication: bool = field(
        default=False, metadata={"help": "Skip P2P communication for the replica."}
    )
    use_fake_page_table_tensor: bool = field(
        default=False, metadata={"help": "Use fake page table tensor for the replica."}
    )
    skip_attention_reduction: bool = field(
        default=False, metadata={"help": "Skip attention reduction for the replica."}
    )
    use_native_execution_backend: bool = field(
        default=False, metadata={"help": "Use native execution backend for the replica."}
    )
