...
class ParallelConfig:
    ...
    pp_dimension: int
    cp_dimension: int
    max_seq_cache_occupancy: int

    def get_key(self):
        return f"tp{self.tp_dimension}_pp{self.pp_dimension}_cp{self.cp_dimension}_co{self.max_seq_cache_occupancy}"

    def get_human_readable_name(self):
        return f"TP: {self.tp_dimension}, PP: {self.pp_dimension}, CP: {self.cp_dimension}: CO: {self.max_seq_cache_occupancy}"

    def get_num_gpus(self):
        return self.tp_dimension * self.pp_dimension * self.cp_dimension

    def to_config_dict(self):
        return {
            "parallel_config_tensor_parallel_size": self.tp_dimension,
            "parallel_config_pipeline_parallel_size": self.pp_dimension,
            "parallel_config_cache_parallel_size": self.cp_dimension,
            "parallel_config_model_max_seq_cache_occupancy": self.max_seq_cache_occupancy,
        }
