...
class BaseWorker:
    ...
    def init_model(self):
        ...
        self.cache_model_parallel_rank = get_cache_model_parallel_rank()
        ...
        logger.info(
            ...
            f"and cache parallel rank {self.cache_model_parallel_rank}."
        )
    ...
    def get_model_parallel_ranks(self) -> Tuple[int, int]:
        return (
            ...
            self.pipeline_model_parallel_rank,
            self.cache_model_parallel_rank,
        )
    ...
    def execute_model(
        ...
        with self.on_schedule_handling_timer:
            _, seq_metadata_list = self.seq_manager.on_schedule(scheduler_output)

        # if len(seq_metadata_list) == 0:
        #     seq = None
        # else:
        #     seq = seq_metadata_list[0].seq

        # print(
        #     f"Worker {self.rank} executing model with {seq_metadata_list} and {seq}", flush=True
        # )

        # torch.cuda.synchronize()

        # print(
        #     f"Worker {self.rank} starting executing model with {seq_metadata_list} and {seq}", flush=True
        # )
...
def _init_distributed_environment(
    ...
    initialize_model_parallel(
        ...
        parallel_config.cache_parallel_size,
    )
