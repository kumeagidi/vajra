...
from sarathi._model_executor_C.model_executor import RotaryEmbedding as RotaryEmbeddingC
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if self.use_native_execution_backend:
            self.native_handler = RotaryEmbeddingC(
                head_size,
                rotary_dim,
    ...
    def forward(
        ...
        if self.use_native_execution_backend:
            self.native_handler.forward(positions, query, key)
            return query, key

        # kernels.rotary_embedding() is an in-place operation that
