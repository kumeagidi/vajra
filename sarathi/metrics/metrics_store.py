...
class MetricsStore:
    ...
    def reset(self):
        ...
        for metric_name in OrderedDataSeries:
            self.ordered_data_series[metric_name] = DataSeries(
                metric_name.value,
                TIME_STR,
            )
    ...
    def _on_request_end(self, seq: Sequence) -> None:
        ...
        if self.config.enable_request_outputs:
            self.requests_outputs.append(
                RequestOutput.from_seq(seq, self.enabled_append_request_execution_stats)
            )
    ...
    def _to_chrome_trace_dict(
        ...
        replica_id = f"{self.replica_id}-cp{cache_parallel_rank}"
    ...
    def _store_completion_metrics(self, base_plot_path: str):
        ...
        for dataseries in self.ordered_data_series.values():
            dataseries.plot_step(
                base_plot_path,
                f"{dataseries.x_name}_time_series",
                y_axis_label=TIME_STR if "time" in dataseries.y_name else COUNT_STR,
    ...
    def merge(self, other: "MetricsStore"):
        ...
        for metric_name in OrderedDataSeries:
            self.ordered_data_series[metric_name].merge(
                other.ordered_data_series[metric_name]
            )
